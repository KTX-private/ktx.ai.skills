...
async function main() {
  ...
  // Check current price to ensure order is reasonable
  try {
    const ticker = await publicClient.getTicker(symbol);
    console.log(`Current market price: ${ticker.last || ticker.last_price} USDT`);
    console.log(`Order price: ${price} USDT\n`);
  } catch (error) {
    console.log('Warning: Could not fetch current price\n');
  }
  ...
  try {
    ...
    if (result.state && result.state < 0) {
      ...
      console.error('\nPossible reasons:');
      console.error('1. Insufficient margin/USDT balance');
      console.error('2. Leverage not allowed or exceeds limit');
      console.error('3. Position size below minimum');
      console.error('4. Account permission issue');
      process.exit(1);
    }
