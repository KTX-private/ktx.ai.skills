...
async function main() {
  ...
  console.log('=== Closing Long Position using Skill Method ===\n');
  ...
  const btcPositions = positions.filter(p =>
    ...
    p.side === 'long' &&  // Only long positions
    parseFloat(p.quantity || '0') > 0  // With non-zero quantity
  );

  if (btcPositions.length === 0) {
    console.log('No BTC_USDT_SWAP long positions found with quantity > 0.\n');

    // Check if there are positions with zero quantity
    const allBtcPositions = positions.filter(p =>
      p.symbol && p.symbol.includes('BTC_USDT_SWAP')
    );

    if (allBtcPositions.length > 0) {
      console.log('Found BTC_USDT_SWAP positions (checking details):');
      allBtcPositions.forEach((pos, index) => {
        console.log(`${index + 1}. Side: ${pos.side}, Quantity: ${pos.quantity}, Closable: ${pos.closableQty}`);
      });
    }
    return;
  }
  ...
  // Close positions using Skill method
  console.log('2. Closing long positions using closeLongPosition() method...\n');
  ...
  for (const pos of btcPositions) {
    ...
    try {
      // Use Skill's closeLongPosition method
      const result = await client.closeLongPosition(symbol, quantity, price, {
